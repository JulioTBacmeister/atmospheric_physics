...
!
    ...
    !
    if (nin.lt.2) then
       STOP ! call endrun('LININTERP: Must have at least 2 input points for interpolation')
    end if
    ...
    if (icount.gt.0) then
       STOP ! call endrun('LININTERP: Non-monotonic input coordinate array found')
    end if
    ...
    if (icount.gt.0) then
       STOP ! call endrun('LININTERP: Point found without interp indices')
    end if
  ...
  subroutine lininterp_original (arrin, yin, nlev, nlatin, arrout, &
    ...
    if (icount.gt.0) then
       STOP ! call endrun('LININTERP: Non-monotonic coordinate array(s) found')
    end if
    !
    ! Initialize index arrays for later checking
    !
  ...
  subroutine bilin (arrin, xin, yin, nlondin, nlonin, &
    ...
    if (icount > 0) then
       STOP ! call endrun ('BILIN: Non-monotonic coordinate array(s) found')
    end if

    if (yout(nlatout) <= yin(1) .or. yout(1) >= yin(nlatin)) then
       STOP ! call endrun ('BILIN: No overlap in y-coordinates')
    end if
    ...
    do j=1,nlatout
       ...
       if (xout(nlonout(j),j) <= xin(1) .or.  &
          ...
          STOP ! call endrun ('BILIN: No overlap in x-coordinates')
       end if
    end do
    !
    ! Initialize index arrays for later checking
    ...
    do j=js,jn
       ...
       STOP ! call endrun ('BILIN: Failed to find interp values')
30     jjprev = jj
    end do
    ...
    if (icount > 0) then
       STOP ! call endrun ('BILIN: something bad in latitude indices or weights')
    end if
    !
...
60        iiprev = ii
  ...
  subroutine vertinterp(ncol, ncold, nlev, pmid, pout, arrin, arrout)
    ...
    !
    if (error) then
       STOP ! call endrun ('VERTINTERP: ERROR FLAG')
    end if
  ...
  subroutine get_timeinterp_factors (cycflag, np1, cdayminus, cdayplus, cday, &
    ...
    if (np1 == 1 .and. .not. cycflag) then
       STOP ! call endrun ('GETFACTORS:'//str//' cycflag false and forward month index = Jan. not allowed')
    end if

    if (np1 < 1) then
       STOP ! call endrun ('GETFACTORS:'//str//' input arg np1 must be > 0')
    end if
